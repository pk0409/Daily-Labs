...
public class EmployeeService {
    ...
    {
    Map<String, List<Employee>> map = EmployeeRepository.getEmployees().stream().collect(Collectors.groupingBy((employee)->employee.getDepartment().getDepartmentName()));
    int max = 0;
    String department ="";
	...
	//EmployeeRepository.getEmployeeList().stream().filter(empMan->empMan.getManager_Id()==null).forEach(System.out :: println);
	...
	public void salaryHike() {
		...
		for ( Employee employee : emp) {
			System.out.println("EmpName: "+employee.getFirstName()
			+"\nSalary increased: "+employee.getSalary()*1.15);
			}
		}
	/*
